#include "parsed_result_handler.h"
...
RawStmt* handle_parsed_result(const uint8_t* buf, size_t len) {
    PgQuery__ParseResult *result = pg_query__parse_result__unpack(NULL, len, buf);
    if (result == NULL) {
        ereport(ERROR,
                (errcode(ERRCODE_PROTOCOL_VIOLATION),
                 errmsg("failed to unpack protobuf message")));
    }

    // if (result->version != PG_VERSION_NUM) {
    //     ereport(ERROR,
    //             (errcode(ERRCODE_PROTOCOL_VIOLATION),
    //              errmsg("version mismatch: expected %d, got %d",
    //                     PG_VERSION_NUM, result->version)));
    // }
    
    PgQuery__RawStmt *stmt = result->stmts[0];

    RawStmt *raw_stmt = _readRawStmt(stmt);
